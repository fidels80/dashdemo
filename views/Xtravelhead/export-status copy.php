...
<div class="export-status">
    ...
    $jobs = (new \yii\db\Query())
        ->from('export_jobs')
        ->where(['progress' => 0])
        ->andWhere(['job_id' => $jobId]) // Usiamo andWhere con la W maiuscola
        ->orderBy(['id' => SORT_ASC])
        ->all();
...
<script>
    ...
    function updateUI(data) {
        ...
        // Se ha finito, scrivi palesemente "COMPLETATO!" al posto del "100%"
        ...
        // Aggiorna status label e pulisce l'interfaccia
        ...
        switch (data.status) {
            case 'completed':
                ...
                $('#performance-info').hide(); // <--- RISOLVE IL PROBLEMA: Nasconde le statistiche congelate
                break;
            case 'failed':
                statusLabel.addClass('label-danger');
                progressBar.removeClass('progress-bar-striped active');
                progressBar.addClass('progress-bar-danger');
                $('#performance-info').hide(); // <--- Nasconde anche in caso di errore
                break;
            case 'processing':
        ...
        if (data.status === 'completed') {
            ...
            // Se mancava il tempo elaborazione, forza la scritta completato
            if (!data.processing_time) {
                $('#perf-processing-time').html('Tempo elaborazione: <span class="text-success">Completato</span>');
            }
    ...
    // --- NUOVO: Aggiungi feedback quando si clicca il bottone di download ---
    $(document).on('click', '#download-link', function() {
        var btn = $(this);
        // Cambia colore e testo per far capire all'utente che l'azione ha funzionato
        btn.removeClass('btn-success').addClass('btn-primary');
        btn.html('<i class="glyphicon glyphicon-ok"></i> File Scaricato! Puoi chiudere questa pagina.');
    });

    function XXXXupdateUI(data) {
        // Aggiorna progress bar
        const progressBar = $('#progress-bar');
        const progressText = $('#progress-text');
        const progress = data.progress || 0;
    ...
    function copyDownloadLink() {
        const downloadUrl = $('#download-link').attr('href');
        if (downloadUrl) {
