...
Yii::error(print_r($dataProvider->getModels(), true), 'debug');

$gridColumns=[
['label'=>'Editore',
'value'=> function ($model, $key, $index, $widget) { // Closure method
                
    $ris= (new yii\db\Query())->select(['Cd_ARMarca'])->from('Rep_publicazioni')
    ->where(['descrizione' => $model->descrizione])
    ->distinct()
    ->one();
    
    return $ris['Cd_ARMarca'];}
