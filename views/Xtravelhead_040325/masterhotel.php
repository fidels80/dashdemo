...
// Se il referrer è assoluto, estraiamo solo il path relativo
if ($referrer) {
    $parsedReferrer = parse_url($referrer, PHP_URL_PATH) . '?' . parse_url($referrer, PHP_URL_QUERY);
} else {
    $parsedReferrer = null;
}

// Debugging (puoi rimuoverlo dopo il test)
//echo 'Current URL: ' . $currentUrl . '<br>';
//echo 'Referrer: ' . $referrer . '<br>';
//echo 'Parsed Referrer: ' . $parsedReferrer . '<br>';

// Confronto solo sulla parte relativa dell'URL
$redirectUrl = ($parsedReferrer && $parsedReferrer !== $currentUrl) ? $referrer : ['index'];

echo Html::a('Torna indietro', $redirectUrl, ['class' => 'btn btn-secondary']);
?>
...
$tttemp=0;
 ...
 echo Html::button($value['citta'] ?? 'N.d.', [
    ...
    'data-target' => '#citta_'.preg_replace('/[^a-zA-Z0-9]/', '', $value['citta'])
        ...
        echo '</td>';
        echo '<td>€ ';
        echo formatEuro($value['real_iva'], 2);
        echo '</td>';
        echo '<td>€ ';
        echo formatEuro($value['totale']+$value['ctax'], 2);
        echo '</td>';
        echo '<td>€ ';
        ...
        $tttemp=$value['imponibile']+$value['ctax']+$tttemp;
        echo formatEuro($value['imponibile']+$value['ctax'], 2);
    ...
    echo '<td style="font-size: 16px;"> <StRONG>€ ';
    echo formatEuro($sommatfatt-$sommaxttax, 2) . '</td>';
    $timp=$sommaEta;
    ...
    $ttax=$sommaEta;
    $eta = array_column($roomlist['tappetour'], 'real_iva');
    ...
    $sommaEta = array_sum($eta);
    echo '<td style="font-size: 16px;"> <StRONG>€ ';
    echo formatEuro($sommaEta+$tctax, 2) . '</td>';
...
?>
 <?php echo $this->render('_modal_cli',['roomlist' => $roomlist]);?>
<?php Modal::end() ?>
...
<?php echo $this->render('_modal_tool',['mdettaglio' => $mdettaglio,
'mth_id'=>$mth_id,
'mpivot'=>$mpivot,
'mlabels'=>$mlabels,
'mseries'=>$mseries,
