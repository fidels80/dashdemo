...
$command3 = $connection->createCommand("select count(*) as tconf
 from doc_head
where cd_cli=:id and confermato=1 ")
    ->bindParam(':id', $ris['cd_cli']);
$resulconf = $command2->queryAll();
$command4 = $connection->createCommand("select count(*) as tconf
 from doc_head
where cd_cli=:id and confermato is null ")
    ->bindParam(':id', $ris['cd_cli']);
$resulwait = $command2->queryAll();
$command5 = $connection->createCommand("select count(*) as tconf
 from doc_head
where cd_cli=:id and annullato=1 ")
    ->bindParam(':id', $ris['cd_cli']);
$resulcanc = $command2->queryAll();

yii::error($ris['cd_cli']);
yii::error($resulwait);
yii::error($resulcanc);
...
//        ],
        ...
        'dataLabels' => [
            'enabled' => True,
            'labels'=> ['Confermato','In attesa','Rifiutato']
        ],
        'stroke' => [
            'show' => true,
            // 'colors' => ['transparent'],
