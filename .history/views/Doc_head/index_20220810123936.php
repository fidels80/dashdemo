...
<div class="doc-head-index">
    ...
    'columns' => [
        ...
        ['label'=>'Intestatario', 'headerOptions' => ['class' => 'card-header bg-' . $usrgrid . ' text-white'],
     'format' => 'text',
            'width' => '350px',
     'value' => function ($model, $key, $index, $widget) {
        yii::warning($model);
        /*        if ($model->altcli<> null ) {
                        $r=anacli::find()->where(['cd_cli'=>$model->altcli])->one();
return $r->Desk;

                }    elseif($model->altcli==null){

$r = anacli::find()->where(['cd_cli' => $model->cd_cli])->one();
return $r->Desk;

                } */
 //              if (isset($model->altcli)){
