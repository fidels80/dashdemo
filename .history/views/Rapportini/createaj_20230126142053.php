...
$sottoc = ArrayHelper::map(
    dosottocommessa::find()
        ->select(['Cd_DOSottoCommessa as id', 
        '(Cd_DOSottoCommessa+\' \'+Descrizione) as desk' ])
        ->orderBy('cd_DoSottoCommessa')->asArray()->all(), 'id', 'desk'
 ...
 <?php   $form = ActiveForm::begin([ 'enableClientValidation' => true,'options' => [
     ...
     ?>
      <?= $form->field($model, 'userid')->textInput(['readonly' => true,'hidden'=>true,
       'value' =>Yii::$app->user->identity->id])->hiddenInput()->label(false)  ?>
