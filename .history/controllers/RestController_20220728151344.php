...
"*/
    public function actionArcapostfile($hash, $content, $id, $fn)
    {
...
'DOTES',
:EntityId,
'PRV  2106     del 25-07-22'
,null
     )"

                )
           //     ->bindValue(':descrizione', 'Caricato da portale web')
           //     ->bindValue(':filename', $fn)
            //    ->bindValue(':EntityId', $id.'.0')
            ...
            'EntityId' => $id])
            // ->where(['id_dotes'=>$id])*/
              //  ->bindValue(':contenuto', '0x', PARAM_LOB);

          ->bindValues(    [':descrizione'=>'Caricato da portale web',
          ':filename'=> $fn,
          ':EntityId'=> $id,
           ':contenuto'=>new PdoValue($content, \PDO::PARAM_LOB)]);
                //$content);
                 //->execute();
