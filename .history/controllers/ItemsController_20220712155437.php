...
$mn=(new \yii\db\Query())
        ...
        ->from('xmenu')
        ->where(['upper(Nmodulo)' => 'ITEMS'])
        ->one();
}

 $arrmod=unserialize($ris['moduli']);
 //yii::error($nmod);
 //yii::error($md['Nmodulo']);

$go=1;
if ($ris['level'] <>100) {
    foreach ($arrmod as  $value) {
        # code...yii::error($arrmod);
    //    yii::error();
        yii::error('---------value----'.strtoupper($value).'---------value----');
   //     yii::error();
        yii::error('--------modulo-----'.strtoupper($mn['voce']).'---------value----');
     //   yii::error('-------------');
        if (strtoupper($value)=strtoupper($md['voce'])) {
         $go=1;
         yii::error('-----sadasdas--------');
