...
$model->save();
    ...
    'op' => $model->className() . '-->' . $this->action->id]);
        ...
        $query2 = yii::$app->db2
    ->createCommand()
    ->update('DOTES', ['xconfermato' => 1, 'xannullato' => 0]
...
$message = Yii::$app->mailer->compose()
    ...
    ->setSubject('Documento Confermato')
    ->setTextBody('Documento Confermato  '.$model->numdoc . 'del '.$model->data
...
$message = Yii::$app->mailer->compose()
    ...
    ->setTextBody('Documento Rifiutato  ' . $model->numdoc . 'del ' . $model->data
        . ' per il cliente ' . $model->cd_cli. '\r  Motivazione: '.$model->rifiutato_nota);
//  ->attach($this->files);
$message->send();
...
if (($model = Doc_head::findOne($id)) !== null) {
    if ($model->load(Yii::$app->request->post())) {
        ...
        $model->setAttribute('dest', $model->dest);
       // $model->setAttribute('rifiutato_nota', $model->rifiutato_nota);
        $t = Yii::$app->runAction('log/set', ['data' => $model,
            'op' => $model->className() . '-->' . $this->action->id]);
