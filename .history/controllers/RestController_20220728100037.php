...
"*/
    public function actionArcapostfile($hash, $content, $id, $fn)
    {
        if ($this->gethash($hash) == false) {
            ...
            $query = yii::$app->db2
                ->createCommand(
"insert into DmsDocument(content,entityTable,descrizione,filename,EntityId,DocumentDate,LinkedToFS)
values
(convert(
            VARBINARY(max) ,  
            :contenuto ),'DOTES',:descrizione,:filename,:EntityId,getdate(),0)
