...
$model->defineAttribute('nota', 'string');
$model->defineAttribute('f_content', 'file');
$model->defineAttribute('origine', 'string');

$model->addRule('nota', 'string');
$model->addRule('f_content', 'file',['maxFiles' => 10]);
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            $tmpfile = UploadedFile::getInstances($model, 'f_content');
$a='';
   ...
   // $file->saveAs(
     ...
     $model = new Allfiles();
      ...
      $a=$a. ((  $file->baseName . '.' . $file->extension));
        ...
        return  print_r($tmpfile);
            
    $h = //'0x'.bin2hex((binary)
            //pack("0x*",    (binary) (file_get_contents($tmpfile->tempName));
            //  var_dump($model);
...
//riattivare
            ...
            $body = "Sono stati caricati dei file " . PHP_EOL . " Sono relativi al documento " . $ldoc['cd_doc'] . ' numero ' . $ldoc['numdoc'] . ' del ' . $ldoc['data'] . PHP_EOL .
                $xn
                . PHP_EOL .
                'Saluti ';
