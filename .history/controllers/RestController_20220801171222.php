...
$nomefile=basename($value->name);
$content = '0x' . bin2hex($h);
...
//file_put_contents(  $file2,  '0x'.bin2hex($h));
...
values
( convert( VARBINARY(max) , :contenuto,1),
        'DOTES',:descrizione,:filename,
       :EntityId
       ,getdate(),0,
       :filesize,
            '\\web\upload\',
...
"
         ...
         ->bindValue(':contenuto','0x' . bin2hex($h)
);
