...
$xnota=$model->xnota;
    ...
    //    DOTES::find()->select('NoteXML')
    //    ->where(['id_dotes'=>$model->xid_testa])->one();
...
declare @tmpxml nvarchar(max)
 ...
 select @tmpxml=cast(NoteXML as varchar(max)) from dotes
 where Id_DoTes=".$xid."
 
select @xmml=cast (@tmpxml as xml)
SET @xmml.modify('replace value of
(rows/row[@nota=(\"6\")]/text())[1] with \"".$xnota."\"')
update dotes set NoteXML=@xmml where Id_DoTes=".$xid
 


    );
    //->bindValues([':xnota' => $xnota,
    // ':xid' =>$xid ]);
  //$query2->bindValues([':id'=>$xid,':nota'=>$xnota]);
    //riattivare
    $query2->execute();
$model->save();
